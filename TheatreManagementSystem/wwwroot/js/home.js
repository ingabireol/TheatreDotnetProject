<<<<<<< HEAD
﻿// home.js - Updated to connect to the ASP.NET backend
...
<<<<<<< HEAD
    apiService.movies.getNowPlaying()
        .then(movies => {
            displayMovies(nowPlayingContainer, movies);

            // Also fetch screenings for each movie
            fetchScreeningsForMovies(movies);
=======
...
<<<<<<< HEAD
    apiService.movies.getUpcoming()
        .then(movies => {
            displayUpcomingMovies(upcomingContainer, movies);
=======
...
function fetchScreeningsForMovies(movies) {
    ...
    movies.forEach(movie => {
<<<<<<< HEAD
        apiService.screenings.getByMovie(movie.id)
=======
...
function getGenreClass(genre) {
    const genreMap = {
        ...
        'COMEDY': 'bg-success',
        'HORROR': 'bg-dark',
        'SCI_FI': 'bg-info text-dark',
        'ROMANCE': 'bg-danger',
        'ANIMATION': 'bg-success',
        'THRILLER': 'bg-dark',
        'FANTASY': 'bg-primary',
        'ADVENTURE': 'bg-warning text-dark'
    };
...
<<<<<<< HEAD
    ...
    if (token) {
        // For a full implementation, you would validate the token with the server
        // or check the expiration from the JWT payload
        updateHeaderUI();
=======
    const token = localStorage.getItem('jwtToken');
    if (token) {
